...
struct Node
{
    ...
    //   nself = this node's primal value is actually needed (read by some
    //   emitted derivative, transitively). A primal that feeds only the
    //   function's value
    //   -- never a derivative -- is dead in a pure derivative/gradient and is
    //   not emitted (so e.g. a `exp(x)` term drops out of d/dy entirely).
...
struct Ctx
{
    ...
    std::vector<info>
        callStack; // callees currently being inlined (cycle guard)
...
consteval std::size_t findSlot(const Ctx &c, std::string_view name)
{
    for (std::size_t i = c.envDecl.size(); i-- > 0;)
        if (m::identifier_of(c.envDecl[i]) == name)
            return c.envSlot[i];
    throw "reflection AD: name is not a parameter or local of the reflected "
          "body";
}
...
{
    if (is_std_fn(callee, ^^p_sin<float>, ^^p_sin<double>,
                  ^^p_sin<long double>))
        return {true, OpKind::Sin};
    if (is_std_fn(callee, ^^p_cos<float>, ^^p_cos<double>,
                  ^^p_cos<long double>))
        return {true, OpKind::Cos};
    if (is_std_fn(callee, ^^p_exp<float>, ^^p_exp<double>,
                  ^^p_exp<long double>))
        return {true, OpKind::Exp};
    if (is_std_fn(callee, ^^p_log<float>, ^^p_log<double>,
                  ^^p_log<long double>))
...
consteval std::size_t lower(Ctx &c, info e)
{
    ...
    if (m::is_literal(e))
    {
        // Reduce the literal to a value reflection (constant_of); it is spliced
        // by the existing P2996 value splice at codegen -- no expression
        // splicing.
        return emit(c, OpKind::Const, 0, 0, m::constant_of(e));
    }
    ...
    if (m::is_binary_operator(e))
    {
        ...
        // Short-circuit: lower the right operand under the guard saying the
        // left did not already decide, so `x != 0 && 1/x > 5` stays safe.
        if (op == OpKind::And || op == OpKind::Or)
        {
            std::size_t outer = c.curGuard;
            std::size_t reached =
                (op == OpKind::And) ? a : emit(c, OpKind::Not, a);
...
consteval std::size_t lower_body(Ctx &c, info body)
{
    ...
    for (info s : m::statements_of(body))
    {
        ...
        {
            if (sawReturn)
                throw "reflection AD: multiple return statements "
        ...
        {
            throw "reflection AD: unsupported statement; straight-line bodies "
                  "only "
...
consteval std::size_t inline_call(Ctx &c, info call)
{
    ...
    // Cycle guard: recursive helpers are out of scope (would not terminate
    // here).
    ...
    // (d) close the scope: drop callee params + locals, restoring the caller
    // env.
...
consteval bool deriv_reads_operand_vals(OpKind op)
{ // reads val[a] (and val[b])
...
consteval bool deriv_reads_self_val(OpKind op)
{ // reads val[self]
                             ...
                             unsigned long long active_mask)
{
    ...
    for (Node &n : ns)
    {
        ...
        if (n.op == OpKind::Input)
            ...
            varied =
                false; // a predicate is piecewise constant: zero derivative
        else
    ...
    // propagate backward through primal dependencies (to compute val[i] you
    // need its operands' vals). Primals that feed only the function value --
    // never a derivative -- stay unneeded and are not emitted.
    ...
    for (Node &n : ns)
    {
        ...
        if (deriv_reads_self_val(n.op))
            need[n.self] = 1;
        // A Select's derivative reads the condition even when its value is
        // dead.
        if (op_has_cond(n.op))
            need[n.cond] = 1;
    }
    for (std::size_t i = N; i-- > 0;)
    { // backward: needed val pulls in operands
        if (!need[i])
            continue;
        const Node &n = ns[i];
        if (op_has_a(n.op))
