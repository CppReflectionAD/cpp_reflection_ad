...
// parameters are tensors, differentiated w.r.t. the *parameters* (not the
// data). The tensor type + ops live in tensor.h; the engine names no concrete
// type.
//
...
static double loss_scalar(const nn::Tensor &x, const nn::Tensor &W1,
                          const nn::Tensor &b1, const nn::Tensor &W2,
                          const nn::Tensor &b2) {
...
int main() {
  ...
  auto grads =
      ad::gradient_wrt<^^nn::loss, Tensor, 1, 2, 3, 4>(x, W1, b1, W2, b2);
  ...
  double h = 1e-6, maxrel = 0.0;
  int fails = 0, total = 0;
  for (int k = 0; k < 4; ++k) {
    for (std::size_t idx = 0; idx < P[k]->d.size(); ++idx) {
      double orig = P[k]->d[idx];
      P[k]->d[idx] = orig + h;
      double Lp = loss_scalar(x, W1, b1, W2, b2);
      P[k]->d[idx] = orig - h;
      double Lm = loss_scalar(x, W1, b1, W2, b2);
      P[k]->d[idx] = orig;
      double rel = std::abs((Lp - Lm) / (2 * h) - grads[k].d[idx]) /
                   (1.0 + std::abs(grads[k].d[idx]));
      maxrel = std::max(maxrel, rel);
      ++total;
      fails += (rel > 1e-6);
    }
    std::printf("  d/d%-2s : %2d values, e.g. grad[0]=% .6f\n", nm[k],
                (int)P[k]->d.size(), grads[k].d[0]);
  }
  std::printf("\nchecked %d parameter-gradient values vs finite differences\n",
              total);
  std::printf("max relative error: %.2e  ->  %s\n", maxrel,
              fails ? "MISMATCH" : "ALL MATCH");
  return fails ? 1 : 0;
}
