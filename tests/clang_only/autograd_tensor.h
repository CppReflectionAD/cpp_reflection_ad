...
std::array<T, sizeof...(Params)> gradient_wrt(const Args &...args) {
  ...
  constexpr auto nodes =
      std::define_static_array(build_marked_nodes<Fn, mask>());
  constexpr auto rnodes =
      std::define_static_array(build_marked_nodes_reversed<Fn, mask>());
  ...
  template for (constexpr auto n : rnodes) {
    if constexpr (n.op == OpKind::Output) {
      ...
      if constexpr (n.va)
        acc(n.a, unbroadcast(adj[n.self], val[n.a]));
      if constexpr (n.vb)
        acc(n.b, unbroadcast(adj[n.self], val[n.b]));
    } else if constexpr (n.op == OpKind::Sub) {
      if constexpr (n.va)
        acc(n.a, unbroadcast(adj[n.self], val[n.a]));
      if constexpr (n.vb)
        acc(n.b, unbroadcast(neg(adj[n.self]), val[n.b]));
    } else if constexpr (n.op == OpKind::Mul) { // elementwise
      if constexpr (n.va)
        acc(n.a, unbroadcast(mul(adj[n.self], val[n.b]), val[n.a]));
      if constexpr (n.vb)
        acc(n.b, unbroadcast(mul(adj[n.self], val[n.a]), val[n.b]));
    } else if constexpr (n.op == OpKind::Div) {
      if constexpr (n.va)
        acc(n.a, unbroadcast(div(adj[n.self], val[n.b]), val[n.a]));
      if constexpr (n.vb)
        acc(n.b, unbroadcast(neg(div(mul(adj[n.self], val[n.a]),
                                     mul(val[n.b], val[n.b]))),
                             val[n.b]));
    } else if constexpr (n.op == OpKind::Neg) {
      if constexpr (n.va)
        acc(n.a, neg(adj[n.self]));
    } else if constexpr (n.op == OpKind::Matmul) {
      // C = A·B  =>  Ā += C̄·Bᵀ ,  B̄ += Aᵀ·C̄
      if constexpr (n.va)
        acc(n.a, matmul(adj[n.self], transpose(val[n.b])));
      if constexpr (n.vb)
        acc(n.b, matmul(transpose(val[n.a]), adj[n.self]));
    } else if constexpr (n.op == OpKind::Transpose) {
      if constexpr (n.va)
        acc(n.a, transpose(adj[n.self]));
    } else if constexpr (n.op == OpKind::Sum) {
      // scalar adjoint broadcast back to the summed tensor's shape
      if constexpr (n.va)
        acc(n.a, mul(ones_like(val[n.a]), adj[n.self]));
    } else if constexpr (n.op == OpKind::Relu) {
      if constexpr (n.va)
        acc(n.a, mul(adj[n.self], d_relu(val[n.a])));
    }
