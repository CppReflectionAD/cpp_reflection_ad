...
static Var push(TOp op, const Var &A, const Var &B, Tensor res, Tensor sa,
                Tensor sb) {
  int id = (int)tape.size();
  tape.push_back({op, A.id, B.id, A.v.r, A.v.c, B.v.r, B.v.c, std::move(sa),
                  std::move(sb)});
...
static Var matmul(const Var &A, const Var &B) {
  return push(TOp::Matmul, A, B, nn::matmul(A.v, B.v), A.v, B.v);
}
static Var add(const Var &A, const Var &B) {
  return push(TOp::Add, A, B, nn::add(A.v, B.v), {}, {});
}
static Var mul(const Var &A, const Var &B) {
  return push(TOp::Mul, A, B, nn::mul(A.v, B.v), A.v, B.v);
}
static Var relu(const Var &A) {
  return push1(TOp::Relu, A, nn::relu(A.v), A.v);
}
...
NOINLINE Grad4 grad(const Tensor &x, const Tensor &W1, const Tensor &b1,
  ...
  tape.clear();
  Var vx = leaf(x), vW1 = leaf(W1), vb1 = leaf(b1), vW2 = leaf(W2),
      vb2 = leaf(b2);
  ...
  auto acc = [&](int i, Tensor g) {
    if (seen[i])
      adj[i] = nn::add(adj[i], g);
    else {
      adj[i] = std::move(g);
      seen[i] = 1;
    }
  };
  adj[L.id] = nn::ones_like(L.v);
  seen[L.id] = 1;
  for (int i = (int)tape.size() - 1; i >= 0; --i) {
    ...
    switch (e.op) {
    case TOp::Matmul:
      acc(e.a, nn::matmul(g, nn::transpose(e.sb)));
      acc(e.b, nn::matmul(nn::transpose(e.sa), g));
      break;
    case TOp::Add:
      acc(e.a, unbroadcast_to(g, e.ar, e.ac));
      acc(e.b, unbroadcast_to(g, e.br, e.bc));
      break;
    case TOp::Mul:
      acc(e.a, unbroadcast_to(nn::mul(g, e.sb), e.ar, e.ac));
      acc(e.b, unbroadcast_to(nn::mul(g, e.sa), e.br, e.bc));
      break;
    case TOp::Relu:
      acc(e.a, nn::mul(g, nn::d_relu(e.sa)));
      break;
    case TOp::Sum:
      acc(e.a, nn::mul(nn::ones_like(e.sa), g));
      break;
    case TOp::Leaf:
      break;
    }
...
static bool close4(const Grad4 &A, const Grad4 &B) {
  for (int k = 0; k < 4; ++k)
    for (std::size_t i = 0; i < A[k].d.size(); ++i)
      if (std::abs(A[k].d[i] - B[k].d[i]) > 1e-6 * (1 + std::abs(B[k].d[i])))
        return false;
  return true;
}
...
static double bench(F fn, Tensor &x, const Tensor &W1, const Tensor &b1,
                    const Tensor &W2, const Tensor &b2, std::size_t iters,
                    int trials) {
  double best = 1e300;
...
int main(int argc, char **argv) {
  ...
  auto fill = [](Tensor &t, double s) {
    for (std::size_t i = 0; i < t.d.size(); ++i)
      t.d[i] = std::sin(s + 0.13 * i) * 0.3;
  };
  fill(x, 0.1);
  fill(W1, 1.0);
  fill(b1, 2.0);
  ...
  std::printf("=== MLP gradient (B=%d In=%d H=%d Out=%d), min ns/call over %d "
              "trials ===\n",
              B, In, H, Out, trials);
  ...
  std::printf("  runtime tape        %9.1f ns   %.2fx\n", t_tape,
              t_tape / t_hand);
  std::printf("\nnet time saved vs runtime tape: %.0f ns/call  (reflection is "
              "%.2fx faster)\n",
