...
template <class F>
inline Tensor ew(const Tensor &a, const Tensor &b,
                 F f) { // elementwise + broadcast
...
inline Tensor d_relu(const Tensor &a) {
  Tensor o = a;
  for (double &v : o.d)
    v = v > 0 ? 1 : 0;
  return o;
...
inline Tensor ones_like(const Tensor &a) {
  Tensor o(a.r, a.c);
  for (double &v : o.d)
    v = 1.0;
  return o;
}
inline Tensor unbroadcast(const Tensor &g,
                          const Tensor &like) { // reverse of broadcasting
  Tensor o(like.r, like.c);
  for (int i = 0; i < g.r; ++i)
    for (int j = 0; j < g.c; ++j)
      o.at(i % like.r, j % like.c) += g.at(i, j);
  return o;
}
