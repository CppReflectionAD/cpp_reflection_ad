...
//                             backward pass (mini "autograd"; requires
//                             rewriting the function in terms of Var).
//   (source generation, e.g. gccad, emits derivative *source* and compiles it,
//    so its run-time cost equals hand-written — same as ours, different
//    workflow.)
//
// Timing is in-process (the derivative is inlined arithmetic; whole-process
// timing would be startup noise). We defeat the optimizer with a sink +
// barrier, vary the input each iteration, keep each variant noinline so
...
NOINLINE double dheavy_f_hand(double x) {
  ...
  double e = d * d + std::log(c * c + 1.0),
         de = 2.0 * d * dd + (2.0 * c * dc) / (c * c + 1.0);
  double g = std::cos(e * 0.5) + e * d,
         dg = -std::sin(e * 0.5) * 0.5 * de + de * d + e * dd;
...
NOINLINE Grad4 grad_heavy_g_reflect_fwd(double a, double b, double c,
                                        double d) {
...
struct Var {
  double v;
  int idx;
  Var(double x) : v(x), idx((int)g_tape.size()) {
    g_tape.push_back(TNode{});
  } // leaf
...
inline Var operator+(Var a, Var b) {
  return mk(a.v + b.v, a.idx, 1.0, b.idx, 1.0);
}
inline Var operator-(Var a, Var b) {
  return mk(a.v - b.v, a.idx, 1.0, b.idx, -1.0);
}
inline Var operator*(Var a, Var b) {
  return mk(a.v * b.v, a.idx, b.v, b.idx, a.v);
}
inline Var operator/(Var a, Var b) {
  return mk(a.v / b.v, a.idx, 1.0 / b.v, b.idx, -a.v / (b.v * b.v));
}
inline Var operator-(Var a) { return mk(-a.v, a.idx, -1.0, -1, 0.0); }
inline Var sin(Var a) {
  return mk(std::sin(a.v), a.idx, std::cos(a.v), -1, 0.0);
}
inline Var cos(Var a) {
  return mk(std::cos(a.v), a.idx, -std::sin(a.v), -1, 0.0);
}
inline Var exp(Var a) {
  return mk(std::exp(a.v), a.idx, std::exp(a.v), -1, 0.0);
}
...
static bool approx(double x, double y) {
  return std::abs(x - y) <= 1e-6 * (1.0 + std::abs(y));
}

template <class F>
static double bench_scalar(F fn, std::size_t iters, int trials) {
  double best = 1e300;
...
template <class F>
static double bench_grad(F fn, std::size_t iters, int trials) {
  double best = 1e300;
  for (int t = 0; t < trials; ++t) {
    double sink = 0.0;
    auto t0 = clk::now();
...
int main(int argc, char **argv) {
  ...
  // correctness
  {
    ...
    Grad4 gh = grad_heavy_g_hand(a, b, c, d),
          gr = grad_heavy_g_reflect(a, b, c, d),
          gt = grad_heavy_g_tape(a, b, c, d),
          gfw = grad_heavy_g_reflect_fwd(a, b, c, d);
    for (int i = 0; i < 4; i++)
      ok = ok && approx(gr[i], gh[i]) && approx(gt[i], gh[i]) &&
           approx(gfw[i], gh[i]);
    if (!ok) {
