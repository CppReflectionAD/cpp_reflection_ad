    ...
    if constexpr (is_unary_inverse_pair<FirstPair>::value)
    {
        ...
        const T inv_err =
            ...
            finite_abs_or_inf(one_wrapped,
                              apply_pair_left(FirstPair{}, inv_one));
        ...
        const T fwd_err =
            ...
            finite_abs_or_inf(one_wrapped,
                              apply_pair_right(FirstPair{}, fwd_one));

        return inv_err <= fwd_err;
    }
    else
    {
        return choose_unary_unwrap_inverse_direction<T, RestPairs...>(
...
constexpr T apply_registered_inverse_wrt(T y, ExtraArgs... args)
{
    if constexpr (pair_matches_wrt<Fn, ArgIndex, FirstPair>::value)
        ...
        return apply_registered_inverse_wrt<Fn, ArgIndex, T, RestPairs...>(
            y, args...);
}
...
template <info Fn, std::size_t ArgIndex> consteval auto build_inverse_plan_wrt()
{
    static constexpr auto nodes = std::define_static_array(build_nodes<Fn>());
    constexpr std::size_t N = nodes.size();
    constexpr std::size_t InputCount = input_count_of<Fn>();
    ...
    else
    {
        ...
        while (true)
        {
                ...
                n.op == OpKind::Mul || n.op == OpKind::Div)
            {
                ...
                if (n.op == OpKind::Add)
                {
                    steps[step_count++] = InverseStepWrt{
                        InverseStepKind::AddConst, static_cast<int>(cst_idx)};
                }
                ...
                {
                    steps[step_count++] = InverseStepWrt{
                        InverseStepKind::MulConst, static_cast<int>(cst_idx)};
                }
                else
                {
                    steps[step_count++] =
                        a_dep ? InverseStepWrt{InverseStepKind::DivConst,
                                               static_cast<int>(cst_idx)}
...
consteval auto build_inverse_plan()
{
    ...
    else
    {
        ...
        // If the whole function is affine with nonzero slope, we can always
        // emit an explicit symbolic inverse: x = (y - b) / a.
        ...
        while (true)
        {
                ...
                n.op == OpKind::Mul || n.op == OpKind::Div)
            {
                ...
                {
                    steps[step_count++] =
                        InverseStep{InverseStepKind::AddConst, c};
                }
                else if (n.op == OpKind::Sub)
                {
                    steps[step_count++] =
                ...
                else if (n.op == OpKind::Mul)
                {
                    if (!is_nonzero(c))
                        return fail(static_cast<int>(cur), n.op);
                    steps[step_count++] =
                        InverseStep{InverseStepKind::MulConst, c};
                }
                else
                {
                    if (a_dep)
                    {
                        if (!is_nonzero(c))
                            return fail(static_cast<int>(cur), n.op);
                        steps[step_count++] =
                            InverseStep{InverseStepKind::DivConst, c};
                    }
                    else
                    {
                        if (!is_nonzero(c))
                            return fail(static_cast<int>(cur), n.op);
                        steps[step_count++] =
                            InverseStep{InverseStepKind::ConstDiv, c};
                    }
                }
...
struct inverse_wrt
{
    ...
    constexpr T operator()(T y, ExtraArgs... args) const
    {
        if constexpr (has_registered_inverse_wrt<Fn, ArgIndex,
                                                 RegisteredPairs...>())
        {
            return apply_registered_inverse_wrt<Fn, ArgIndex, T,
                                                RegisteredPairs...>(y, args...);
        }
        else if constexpr (detail_inv::build_inverse_plan_wrt<Fn, ArgIndex>()
                               .ok)
        {
            constexpr auto plan =
                detail_inv::build_inverse_plan_wrt<Fn, ArgIndex>();
            const auto node_values =
                detail_inv::eval_node_values_with_arg<Fn, ArgIndex, T>(T{0},
                                                                       args...);
            T x = y;
            for (std::size_t i = 0; i < plan.step_count; ++i)
        ...
        else if constexpr (has_unary_inverse_pair<RegisteredPairs...>())
        {
            ...
            const T b_wrapped =
                detail_inv::eval_fn_with_arg_runtime<Fn, ArgIndex, T>(T{0},
                                                                      args...);
            const T one_wrapped =
                detail_inv::eval_fn_with_arg_runtime<Fn, ArgIndex, T>(T{1},
                                                                      args...);
            ...
            const T y_unwrapped =
                use_inverse_direction
                    ? apply_registered_unary_transform<true, T,
                                                       RegisteredPairs...>(y)
                    : apply_registered_unary_transform<false, T,
                                                       RegisteredPairs...>(y);
            const T b =
                use_inverse_direction
                    ? apply_registered_unary_transform<true, T,
            ...
            // Prevent unused-variable warnings while keeping diagnostics
            // available when debugging this branch.
            (void)y_unwrapped;
            (void)b;
            (void)one_unwrapped;
            ...
            for (int i = 0; i < 80; ++i)
            {
                ...
                const T fmid =
                    detail_inv::eval_fn_with_arg_runtime<Fn, ArgIndex, T>(
                        mid, args...);
        ...
        {
            static_assert(
                has_registered_inverse_wrt<Fn, ArgIndex,
                                           RegisteredPairs...>() ||
                    detail_inv::build_inverse_plan_wrt<Fn, ArgIndex>().ok ||
                    has_unary_inverse_pair<RegisteredPairs...>(),
                "No inverse_wrt path available: provide inverse_pair_wrt "
                "or a compatible unary inverse_pair, or use a structurally "
                "invertible function wrt the target argument.");
            return y;
        }
    }
};
